...
const tabs = [
  ...
  { label: "마이페이지", href: "/profile", icon: UserRound },
];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const shouldHideShell =
    pathname === "/" || shellHiddenPrefixes.some((path) => pathname.startsWith(path));
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-[430px] flex-col overflow-hidden bg-background shadow-2xl ring-1 ring-white/10 sm:my-6 sm:min-h-[860px] sm:rounded-[36px]">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 mx-auto w-full max-w-[430px] px-3 pb-3 sm:bottom-6">
